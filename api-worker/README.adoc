...
Generate models from Gondul API via

```
curl -sS http://localhost:8000/api/openapi.json | uv run datamodel-codegen --input-file-type openapi --output gondul_models.py
```

if self-hosting the API, or replace http://localhost:8000 with a gondul API somewhere else.
