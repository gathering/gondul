...
# ping
@router.get("/ping")
async def devices(request: Request, response: Response, cache = Depends(get_redis)) -> Ping:
    ...
    for device in ping:
        latency4 = round(ping[device]["v4_rtt"] * 1000, 2) if "v4_rtt" in ping[device] and ping[device]["v4_rtt"] is not None else None
        latency6 = round(ping[device]["v6_rtt"] * 1000, 2) if "v6_rtt" in ping[device] and ping[device]["v6_rtt"] is not None else None
        age4 = (time.time() - ping[device]["v4_time"]) if "v4_time" in ping[device] else None
        age6 = (time.time() - ping[device]["v6_time"]) if "v6_time" in ping[device] else None
        output.update({
            device: {
                "latency4": latency4,
                "latency6": latency6,
