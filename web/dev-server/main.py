...
async def tile_post_request(path: str, request: Request, response: Response):
    async with httpx.AsyncClient() as client:
        proxy = await client.post(f"{os.environ.get("GONDUL_URL")}/api/{path}", auth=auth, data=await request.body())
    response.body = proxy.content
    response.status_code = proxy.status_code
    return response
